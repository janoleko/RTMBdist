...
Bivariate copulas can be implemented in a modular way using the [`dcopula`](../reference/dcopula) function together with one of the copula constructors below. Available copula constructors are:
...
For bivariate copulas with *discrete* margins, use the [`ddcopula`](../reference/ddcopula) function instead. In this case, instead of copula *densities*, copula *CDFs* are needed. The available constructors for this are:
...
Multivariate copulas are also possible using the [`dmvcopula`](../reference/dmvcopula) function together with one of the multivariate copula constructors below. Currently, only the multivariate Gaussian copula is implemented in two ways:
